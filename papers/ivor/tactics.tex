...
The central data type is \hdecl{Context}, which is an abstract type
...
The \hdecl{Context} structure keeps track of the current proof
state. Initially, there is no proof in progress, so no proof state ---
the \hdecl{theorem} function creates a new proof with the given goal.
...
There are four basic operations on holes, \demph{claim}, \demph{fill},
\demph{abandon} and \demph{solve}; these are given the following types:
\begin{verbatim}
...
hole. The \hdecl{fill} function a the guess to attach to the current
...
have any computational force. The \hdecl{solve} tactic is provided to
...
so.  A guess can be removed from a goal with the \hdecl{abandon}
tactic.
...
\Ivor{} is supplied with a library containing the appropriate
